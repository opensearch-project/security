...
public class SSLRequestHelper {
    ...
    public static SSLInfo getSSLInfo(
        ...
        final SslHandler sslhandler = (SslHandler) ((Netty4HttpChannel) request.getHttpChannel()).getNettyChannel()
            .pipeline()
            .get("ssl_http");
    ...
    private static boolean validate(X509Certificate[] x509Certs, final Settings settings, final Path configPath) {
        ...
        try {
            ...
            if (truststore != null) {
                ...
                try (
                    FileInputStream fin = new FileInputStream(new File(env.configFile().resolve(truststore).toAbsolutePath().toString()))
                ) {
                    ts.load(
                        fin,
                ...
                final File trustedCas = env.configFile()
