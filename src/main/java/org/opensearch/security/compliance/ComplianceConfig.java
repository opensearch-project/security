...
public class ComplianceConfig {
    ...
    public static ComplianceConfig from(Map<String, Object> properties, @JacksonInject Settings settings) throws JsonProcessingException {
        ...
        final boolean enabled = DefaultObjectMapper.getOrDefault(properties, "enabled", true);
        final boolean logExternalConfig = DefaultObjectMapper.getOrDefault(properties, "external_config", false);
        final boolean logInternalConfig = DefaultObjectMapper.getOrDefault(properties, "internal_config", false);
        final boolean logReadMetadataOnly = DefaultObjectMapper.getOrDefault(properties, "read_metadata_only", false);
        final Map<String, List<String>> watchedReadFields = DefaultObjectMapper.getOrDefault(properties, "read_watched_fields", Collections.emptyMap());
        final Set<String> ignoredComplianceUsersForRead = ImmutableSet.copyOf(DefaultObjectMapper.getOrDefault(properties, "read_ignore_users", AuditConfig.DEFAULT_IGNORED_USERS));
        final boolean logWriteMetadataOnly = DefaultObjectMapper.getOrDefault(properties, "write_metadata_only", false);
        final boolean logDiffsForWrite = DefaultObjectMapper.getOrDefault(properties, "write_log_diffs", false);
        final List<String> watchedWriteIndicesPatterns = DefaultObjectMapper.getOrDefault(properties, "write_watched_indices", Collections.emptyList());
        final Set<String> ignoredComplianceUsersForWrite = ImmutableSet.copyOf(DefaultObjectMapper.getOrDefault(properties, "write_ignore_users", AuditConfig.DEFAULT_IGNORED_USERS));
