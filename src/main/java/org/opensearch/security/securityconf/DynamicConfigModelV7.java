...
public class DynamicConfigModelV7 extends DynamicConfigModel {
    ...
    private void buildAAA() {
        ...
        final ConfigV7.Authz authzDyn = config.dynamic.authz;

        for (final Entry<String, ConfigV7.AuthzDomain> ad : authzDyn.getDomains().entrySet()) {
        ...
        final ConfigV7.Authc authcDyn = config.dynamic.authc;

        for (final Entry<String, ConfigV7.AuthcDomain> ad : authcDyn.getDomains().entrySet()) {
            final boolean httpEnabled = ad.getValue().http_enabled;
            final boolean transportEnabled = ad.getValue().transport_enabled;
