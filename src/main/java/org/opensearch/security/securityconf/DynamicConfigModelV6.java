...
public class DynamicConfigModelV6 extends DynamicConfigModel {
    ...
    private void buildAAA() {
        ...
        final ConfigV6.Authz authzDyn = config.dynamic.authz;

        for (final Entry<String, ConfigV6.AuthzDomain> ad : authzDyn.getDomains().entrySet()) {
        ...
        final ConfigV6.Authc authcDyn = config.dynamic.authc;

        for (final Entry<String, ConfigV6.AuthcDomain> ad : authcDyn.getDomains().entrySet()) {
            final boolean enabled = ad.getValue().enabled;
            final boolean httpEnabled = enabled && ad.getValue().http_enabled;
            final boolean transportEnabled = enabled && ad.getValue().transport_enabled;
