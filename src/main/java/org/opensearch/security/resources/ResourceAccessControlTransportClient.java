...
/**
 * Access control transport client responsible for handling resource sharing operations such as verifying,
 * sharing, revoking, and listing access to shareable resources.
 * NOTE: The calls are gated behind `cluster:admin/security/resource_access/*` action permissions
 *
 * @opensearch.experimental
 */
public final class ResourceAccessControlTransportClient implements ResourceSharingClient {

    private static final Logger log = LogManager.getLogger(ResourceAccessControlTransportClient.class);
     ...
     */
    public ResourceAccessControlTransportClient(Client client, Settings settings, ClusterService clusterService) {
        this.client = client;
    ...
    public <T extends ShareableResource> void listAllAccessibleResources(String resourceIndex, ActionListener<Set<T>> listener) {
        ...
        client.execute(ListAccessibleResourcesAction.INSTANCE, request, ActionListener.wrap(response -> {
            listener.onResponse((Set<T>) response.getResources());
        }, listener::onFailure));
    }
