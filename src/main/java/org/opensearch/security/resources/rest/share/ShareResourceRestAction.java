...
public class ShareResourceRestAction extends BaseRestHandler {
    private static final Logger LOGGER = LogManager.getLogger(ShareResourceRestAction.class);

    public ShareResourceRestAction() {}

    @Override
    public List<Route> routes() {
        return addRoutesPrefix(ImmutableList.of(new Route(POST, "/share")), PLUGIN_RESOURCE_ROUTE_PREFIX);
    }

    @Override
    public String getName() {
        return "share_resource_api_action";
    }
    ...
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        ShareResourceRequest resourceAccessRequest = ShareResourceRequest.from(source, request.params());
        return channel -> {
            client.executeLocally(ShareResourceAction.INSTANCE, resourceAccessRequest, new ActionListener<>() {

                @Override
                public void onResponse(ShareResourceResponse response) {
                    try {
     ...
     */
    private void sendResponse(RestChannel channel, ShareResourceResponse response) throws IOException {
