...
public class ShareResourceTransportAction extends HandledTransportAction<ShareResourceRequest, ShareResourceResponse> {
    ...
    @Override
    protected void doExecute(Task task, ShareResourceRequest request, ActionListener<ShareResourceResponse> actionListener) {
    ...
    private void handleGrantAccess(ShareResourceRequest request, ActionListener<ShareResourceResponse> listener) {
        resourceAccessHandler.shareWith(
            request.getResourceId(),
            request.getResourceIndex(),
            request.getShareWith(),
            ActionListener.wrap(response -> listener.onResponse(new ShareResourceResponse(response)), listener::onFailure)
        );
    }
}
