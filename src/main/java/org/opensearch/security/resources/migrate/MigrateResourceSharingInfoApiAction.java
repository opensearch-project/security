...
public class MigrateResourceSharingInfoApiAction extends AbstractApiAction {
    ...
    private void handleMigrate(RestChannel channel, RestRequest request, Client client) throws IOException {
        withUserAndRemoteAddress()
            ...
            .map(pair -> {
                if (endpointValidator.isCurrentUserAdmin()) {
                    ...
                    return ValidationResult.error(
                        ...
                        payload(
                            RestStatus.FORBIDDEN,
                            "User " + pair.getLeft().getName() + " is not allowed to migrate resource sharing info."
                        )
                    );
                }
            })
            // 2. read the old sharing docs
            .map(pair -> loadCurrentSharingInfo(request, client))
            // 3. transform and index the new ResourceSharing records
    ...
    private ValidationResult<List<SourceDoc>> loadCurrentSharingInfo(RestRequest request, Client client) throws IOException {
        ...
        if (body == null || !body.has("source_index") || body.get("source_index").asText().isEmpty()) {
            return ValidationResult.error(
                RestStatus.BAD_REQUEST,
                payload(RestStatus.BAD_REQUEST, "must provide non-empty \"source_index\" in request body")
            );
        }
        ...
        for (SearchHit hit : resp.getHits().getHits()) {
            ...
            if (backendRolesNode.isTextual() && !backendRolesNode.asText().isEmpty()) {
                for (String r : backendRolesNode.asText().split(",")) {
                    r = r.trim();
                    if (!r.isEmpty()) {
                        backendRoles.add(r);
                    }
