...
public class AuditConfig {
    ...
    public static class Filter {
        ...
        public static Filter from(Map<String, Object> properties) throws JsonProcessingException {
            ...
            final boolean isRestApiAuditEnabled = DefaultObjectMapper.getOrDefault(properties,"enable_rest", true);
            final boolean isTransportAuditEnabled = DefaultObjectMapper.getOrDefault(properties,"enable_transport", true);
            final boolean resolveBulkRequests = DefaultObjectMapper.getOrDefault(properties, "resolve_bulk_requests", false);
            final boolean logRequestBody = DefaultObjectMapper.getOrDefault(properties, "log_request_body", true);
            final boolean resolveIndices = DefaultObjectMapper.getOrDefault(properties, "resolve_indices", true);
            final boolean excludeSensitiveHeaders = DefaultObjectMapper.getOrDefault(properties, "exclude_sensitive_headers", true);
            final Set<AuditCategory> disabledRestCategories = AuditCategory.parse(DefaultObjectMapper.getOrDefault(properties,"disabled_rest_categories", ConfigConstants.OPENDISTRO_SECURITY_AUDIT_DISABLED_CATEGORIES_DEFAULT));
            final Set<AuditCategory> disabledTransportCategories = AuditCategory.parse(DefaultObjectMapper.getOrDefault(properties, "disabled_transport_categories", ConfigConstants.OPENDISTRO_SECURITY_AUDIT_DISABLED_CATEGORIES_DEFAULT));
            final Set<String> ignoredAuditUsers = ImmutableSet.copyOf(DefaultObjectMapper.getOrDefault(properties, "ignore_users", DEFAULT_IGNORED_USERS));
            final Set<String> ignoreAuditRequests = ImmutableSet.copyOf(DefaultObjectMapper.getOrDefault(properties, "ignore_requests", Collections.emptyList()));
