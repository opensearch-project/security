...
public final class AuditLogImpl implements AuditLog {
    ...
    private static final List<String> writeClasses = ImmutableList.of(
    ...
    private void onAuditConfigFilterChanged(AuditConfig.Filter auditConfigFilter) {
        this.auditConfigFilter = auditConfigFilter;
        this.auditConfigFilter.log(log);
    }

    private void onComplianceConfigChanged(ComplianceConfig complianceConfig) {
    ...
    private void save(final AuditMessage msg) {
        if (enabled) {
    ...
    public void logFailedLogin(String effectiveUser, boolean securityAdmin, String initiatingUser, TransportRequest request, Task task) {
        if (enabled) {
            ...
            final List<AuditMessage> msgs = RequestResolver.resolve(AuditCategory.FAILED_LOGIN, getOrigin(), action, null, effectiveUser, securityAdmin, initiatingUser, remoteAddress, request, getThreadContextHeaders(), task, resolver, clusterService, this.settings, auditConfigFilter.shouldLogRequestBody(), auditConfigFilter.shouldResolveIndices(), auditConfigFilter.shouldResolveBulkRequests(), opendistrosecurityIndex, auditConfigFilter.shouldExcludeSensitiveHeaders(), null);
    ...
    public void logSSLException(TransportRequest request, Throwable t, String action, Task task) {
        if (enabled) {
            ...
            final List<AuditMessage> msgs = RequestResolver.resolve(AuditCategory.SSL_EXCEPTION, Origin.TRANSPORT, action, null, getUser(), false, null, remoteAddress, request,
                    getThreadContextHeaders(), task, resolver, clusterService, this.settings, auditConfigFilter.shouldLogRequestBody(), auditConfigFilter.shouldResolveIndices(), auditConfigFilter.shouldResolveBulkRequests(), opendistrosecurityIndex, auditConfigFilter.shouldExcludeSensitiveHeaders(), t);
                        ...
                        if(opendistrosecurityIndex.equals(index) && !"tattr".equals(id)) {
                            try {
                                Map<String, String> map = fieldNameValues.entrySet().stream()
                                        .collect(Collectors.toMap(entry -> "id", entry -> new String(BaseEncoding.base64().decode(((Map.Entry<String, String>) entry).getValue()), StandardCharsets.UTF_8)));
    ...
    private void logExternalConfig() {
        if (enabled) {
            final ComplianceConfig complianceConfig = getComplianceConfig();
            ...
            final Map<String, Object> configAsMap = Utils.convertJsonToxToStructuredMap(this.settings);
