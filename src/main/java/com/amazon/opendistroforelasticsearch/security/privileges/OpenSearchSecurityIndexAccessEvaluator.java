...
public class OpenSearchSecurityIndexAccessEvaluator {
    ...
    public OpenSearchSecurityIndexAccessEvaluator(final Settings settings, AuditLog auditLog, IndexResolverReplacer irr) {
    ...
    public PrivilegesEvaluatorResponse evaluate(final ActionRequest request, final Task task, final String action, final Resolved requestedResolved,
        ...
        if ((requestedResolved.getAllIndices().contains(openSearchSecurityIndex) || matchAnySystemIndices(requestedResolved))
            ...
            if(filterSecurityIndex) {
                ...
                allWithoutSecurity.remove(openSearchSecurityIndex);
                if(allWithoutSecurity.isEmpty()) {
                    if (isDebugEnabled) {
                        log.debug("Filtered '{}' but resulting list is empty", openSearchSecurityIndex);
                    }
                ...
                if (isDebugEnabled) {
                    log.debug("Filtered '{}', resulting list is {}", openSearchSecurityIndex, allWithoutSecurity);
                }
                return presponse;
            } else {
                auditLog.logSecurityIndexAttempt(request, action, task);
                log.warn("{} for '{}' index is not allowed for a regular user", action, openSearchSecurityIndex);
                presponse.allowed = false;
                return presponse.markComplete();
            }
        }
        ...
        if (requestedResolved.isLocalAll()
                && securityDeniedActionMatcher.test(action)) {
            if(filterSecurityIndex) {
                irr.replace(request, false, "*","-"+openSearchSecurityIndex);
                if (isDebugEnabled) {
                    log.debug("Filtered '{}'from {}, resulting list with *,-{} is {}", openSearchSecurityIndex, requestedResolved, openSearchSecurityIndex, irr.resolveRequest(request));
                }
                return presponse;
            } else {
                auditLog.logSecurityIndexAttempt(request, action, task);
        ...
        if(requestedResolved.getAllIndices().contains(openSearchSecurityIndex) || requestedResolved.isLocalAll()
