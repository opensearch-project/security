...
import com.amazon.opendistroforelasticsearch.security.ssl.rest.NodeCertInfoAction;
...
public final class OpenDistroSecurityPlugin extends OpenDistroSecuritySSLPlugin implements ClusterPlugin, MapperPlugin {
    ...
    private boolean sslCertReloadable;
    ...
    private static boolean isSslCertReloadable(final Settings settings) {
        return !isDisabled(settings) && !isSslOnlyMode(settings) && settings.getAsBoolean(ConfigConstants.OPENDISTRO_SECURITY_SSL_CERT_RELOAD, false);
    }
    ...
    public OpenDistroSecurityPlugin(final Settings settings, final Path configPath) {
        ...
        sslCertReloadable = isSslCertReloadable(settings);
    ...
    public List<RestHandler> getRestHandlers(Settings settings, RestController restController, ClusterSettings clusterSettings,
        ...
        if (!client && !disabled) {
            ...
            if(!sslOnly) {
                ...
                if (sslCertReloadable) {
                    handlers.add(new SSLCertReloadAction(settings, restController, odsks, Objects.requireNonNull(threadPool), Objects.requireNonNull(adminDns)));
                    handlers.add(new NodeCertInfoAction(settings, restController, odsks, Objects.requireNonNull(threadPool), Objects.requireNonNull(adminDns)));
                }
    ...
    public List<Setting<?>> getSettings() {
        ...
        if(!sslOnly) {
            ...
            settings.add(Setting.boolSetting(ConfigConstants.OPENDISTRO_SECURITY_SSL_CERT_RELOAD, false, Property.NodeScope, Property.Filtered));
        }
