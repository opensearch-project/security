...
public class AuditMessagePredicate implements Predicate<AuditMessage> {
	...
	private AuditMessagePredicate(AuditCategory category, Origin requestLayer, String restRequestPath,
		String initiatingUser, Method requestMethod, String transportRequestType, String effectiveUser, String index) {
	...
	public static AuditMessagePredicate grantedPrivilege(User user, String requestType) {
		return auditPredicate(GRANTED_PRIVILEGES).withLayer(Origin.TRANSPORT).withEffectiveUser(user)
			.withTransportRequestType(requestType);
	}

	public static AuditMessagePredicate missingPrivilege(User user, String requestType) {
		return auditPredicate(MISSING_PRIVILEGES).withLayer(Origin.TRANSPORT).withEffectiveUser(user)
			.withTransportRequestType(requestType);
	}

	public AuditMessagePredicate withLayer(Origin layer) {
		return new AuditMessagePredicate(category, layer, restRequestPath, initiatingUser, requestMethod,
			transportRequestType, effectiveUser, index);
	}

	public AuditMessagePredicate withRequestPath(String path) {
		return new AuditMessagePredicate(category, requestLayer, path, initiatingUser, requestMethod, transportRequestType, effectiveUser,
			index);
	}

	public AuditMessagePredicate withInitiatingUser(String user) {
		return new AuditMessagePredicate(category, requestLayer, restRequestPath, user, requestMethod, transportRequestType, effectiveUser,
			index);
	}
	...
	public AuditMessagePredicate withRestMethod(Method method) {
		return new AuditMessagePredicate(category, requestLayer, restRequestPath, initiatingUser, method,
			transportRequestType, effectiveUser, index);
	}

	public AuditMessagePredicate withTransportRequestType(String type) {
		return new AuditMessagePredicate(category, requestLayer, restRequestPath, initiatingUser, requestMethod, type, effectiveUser, index);
	}

	public AuditMessagePredicate withEffectiveUser(String user) {
		return new AuditMessagePredicate(category, requestLayer, restRequestPath, initiatingUser, requestMethod, transportRequestType, user,
			index);
	}
	...
	public AuditMessagePredicate withIndex(String indexName) {
		return new AuditMessagePredicate(category, requestLayer, restRequestPath, initiatingUser, requestMethod, transportRequestType,
			effectiveUser, indexName);
	}
	...
	public String toString() {
		return "AuditMessagePredicate{" + "category=" + category + ", requestLayer=" + requestLayer + ", restRequestPath='" + restRequestPath + '\'' + ", requestInitiatingUser='" + initiatingUser + '\'' + ", requestMethod=" + requestMethod + ", transportRequestType='" + transportRequestType + '\'' + '}';
	}
}
