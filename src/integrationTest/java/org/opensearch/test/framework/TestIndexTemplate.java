...
public class TestIndexTemplate {
    ...
    public void create(Client client) throws Exception {
        try (XContentBuilder builder = JsonXContent.contentBuilder().map(getAsMap())) {
            try (XContentParser parser = JsonXContent.jsonXContent.createParser(
                ...
                client.admin().indices().execute(PutComposableIndexTemplateAction.INSTANCE,
                        new PutComposableIndexTemplateAction.Request(name).indexTemplate(ComposableIndexTemplate.parse(parser))).actionGet();
            }
        }
    }
    ...
    public Map<String, ?> getAsMap() {
        return ImmutableMap.of(
                ...
                "composed_of", composedOf.stream().map(TestComponentTemplate::getName).collect(Collectors.toList())
        );
    }
}
