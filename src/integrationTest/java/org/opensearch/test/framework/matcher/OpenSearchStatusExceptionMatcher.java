...
class OpenSearchStatusExceptionMatcher extends TypeSafeDiagnosingMatcher<Throwable> {
	...
	protected boolean matchesSafely(Throwable throwable, Description mismatchDescription) {
		if((throwable instanceof OpenSearchException) == false) {
			mismatchDescription.appendText("actual exception type is ").appendValue(throwable.getClass().getCanonicalName())
				.appendText(", error message ").appendValue(throwable.getMessage());
			return false;
		}
		...
		if(expectedRestStatus.equals(openSearchException.status()) == false) {
			mismatchDescription.appendText("actual status code is ").appendValue(openSearchException.status())
				.appendText(", error message ").appendValue(throwable.getMessage());
			return false;
		}
		return true;
