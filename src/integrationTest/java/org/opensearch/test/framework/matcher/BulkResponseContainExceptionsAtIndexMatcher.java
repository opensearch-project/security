...
class BulkResponseContainExceptionsAtIndexMatcher extends TypeSafeDiagnosingMatcher<BulkResponse> {
	...
	protected boolean matchesSafely(BulkResponse response, Description mismatchDescription) {
		...
		if(failure == null) {
			mismatchDescription.appendText("bulk response item with index ").appendValue(errorIndex).appendText(" does not contain failure");
			return false;
		}
		Exception exception = failure.getCause();
		if(exception == null) {
			mismatchDescription.appendText("bulk response item with index ").appendValue(errorIndex).appendText(" does not contain exception.");
			return false;
		}
		if(exceptionMatcher.matches(exception) == false) {
			mismatchDescription.appendText("bulk response item with index ").appendValue(errorIndex)
				.appendText(" contains incorrect exception which is ").appendValue(exception);
			return false;
		}
