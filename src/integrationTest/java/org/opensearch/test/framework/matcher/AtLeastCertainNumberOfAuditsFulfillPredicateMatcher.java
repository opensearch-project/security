...
class AtLeastCertainNumberOfAuditsFulfillPredicateMatcher extends AuditsFulfillPredicateMatcher {
	...
	protected boolean matchesSafely(List<AuditMessage> audits, Description mismatchDescription) {
		...
		if(count < minimumNumberOfAudits) {
			mismatchDescription.appendText(" only ").appendValue(count).appendText(" match predicate. Examined audit logs ")
	...
	public void describeTo(Description description) {
		description.appendText("Al least ").appendValue(minimumNumberOfAudits).appendText(" audits records should match predicate ")
