...
public class IndexAuthorizationReadWriteIntTests {
        ...
        try (TestRestClient restClient = cluster.getRestClient(user); TestRestClient adminRestClient = cluster.getAdminCertRestClient()) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                ...
                if (user != LIMITED_USER_NONE && user != LIMITED_READ_ONLY_ALL && user != LIMITED_READ_ONLY_A) {
                    assertThat(httpResponse, isOk());
                    int expectedDeleteCount = containsExactly(index_aw1, index_bw1).at("_index")
    ...
    public void putDocument_alias() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                ...
                if (user.indexMatcher("write").coversAll(alias_ab1w)) {
                    assertThat(httpResponse, isCreated());
                } else {
                    assertThat(httpResponse, isForbidden());
                }
    ...
    public void createIndex_systemIndex() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.systemIndexPrivilegeEnabled && user.indexMatcher("create_index").covers(system_index_plugin_not_existing)) {
                assertThat(httpResponse, isOk());
            } else if (user == SUPER_UNLIMITED_USER
    ...
    public void createIndex_withAlias() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                ...
                if (user.indexMatcher("manage_alias").coversAll(alias_bwx, index_bwx1)) {
                    assertThat(httpResponse, isOk());
                    assertThat(
                        httpResponse,
                        containsExactly(index_bwx1).at("index").reducedBy(user.indexMatcher("create_index")).whenEmpty(isForbidden())
    ...
    public void deleteAlias_wildcard() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                ...
                if (user.indexMatcher("manage_alias").coversAll(ALL_NON_HIDDEN_INDICES)) {
                    assertThat(httpResponse, isOk());
                } else {
                    assertThat(httpResponse, isForbidden());
                }
    ...
    public void aliases_createAlias() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                ...
                if (user.indexMatcher("manage_alias").coversAll(alias_bwx, index_bw1)) {
                    assertThat(httpResponse, isOk());
                } else {
                    assertThat(httpResponse, isForbidden());
                }
    ...
    public void closeIndex_wildcard() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig.legacyPrivilegeEvaluation) {
                if (user.indexMatcher("manage_index").coversAll(ALL_NON_HIDDEN_INDICES)) {
                    assertThat(httpResponse, isOk());
                } else {
                    assertThat(httpResponse, isForbidden());
                }
                ...
                if (!user.indexMatcher("manage_index").isEmpty()) {
                    assertThat(
                        ...
                        containsExactly(ALL_NON_HIDDEN_INDICES).at("indices.keys()")
                            .reducedBy(user.indexMatcher("manage_index"))
                            .whenEmpty(isOk())
                    );
                } else {
                    assertThat(httpResponse, isForbidden());
