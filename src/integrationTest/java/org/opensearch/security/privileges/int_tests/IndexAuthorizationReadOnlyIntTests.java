...
public class IndexAuthorizationReadOnlyIntTests {
        ...
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            assertThat(
                ...
                containsExactly(index_a1, index_a2, index_a3, index_b1, index_b2, index_b3, index_c1).at("hits.hits[*]._index")
                    ...
                    .whenEmpty(clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION ? 403 : 200)
            );
        }
    }
    ...
    public void search_staticIndicies() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            assertThat(httpResponse, containsExactly(index_a1).at("hits.hits[*]._index").but(user.indexMatcher("search")).whenEmpty(403));
        }
    }
        ...
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION) {
                ...
                if (user == UNLIMITED_USER || user == SUPER_UNLIMITED_USER) {
                    ...
                    // This is actually a bug; the legacy code ignores the expand_wildcards=none option and resolves
                    // to the indices index_a1, index_a2, index_b1. Thus, these users get not exactly what they requested.
                    // Still, they only are allowed to access data which they have privileges for.
                    // Fixing this in the legacy code might be difficult, but this is fixed in the new code (see below)
                    assertThat(httpResponse, isOk());
                } else {
                    assertThat(httpResponse, isForbidden("/error/root_cause/0/reason", "no permissions for [indices:data/read/search]"));
                }
    ...
    public void search_aliasAndIndex_ignoreUnavailable() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION) {
                ...
                if (user.indexMatcher("search").covers(alias_ab1)) {
                    assertThat(httpResponse, containsExactly(index_a1, index_a2, index_a3, index_b1).at("hits.hits[*]._index"));
                } else if (user.indexMatcher("search").covers(index_b1)) {
    ...
    public void cat_all_includeHidden() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION && user == UNLIMITED_USER) {
    ...
    public void getAlias_aliasPattern() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION) {
                ...
                assertThat(
                    httpResponse,
                    containsExactly(index_a1, index_a2, index_a3, index_b1).at("$.keys()")
                        .but(user.indexMatcher("get_alias"))
                        .whenEmpty(403)
                );
            } else {
                if (user.indexMatcher("get_alias").covers(alias_ab1)) {
                    assertThat(httpResponse, isOk());
    ...
    public void resolve_wildcard() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            assertThat(
                httpResponse,
                containsExactly(index_a1, index_a2, index_a3, index_b1, index_b2, index_b3, index_c1, alias_ab1, alias_c1).at("$.*[*].name")
                    .but(user.indexMatcher("read"))
                    .whenEmpty(clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION ? 403 : 200)
            );
        }
        ...
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            assertThat(
                ...
                containsExactly(index_b1, index_b2, index_b3).at("indices")
                    .but(user.indexMatcher("read"))
                    .whenEmpty(clusterConfig == ClusterConfig.NEXT_GEN_PRIVILEGES_EVALUATION ? 200 : 403)
            );
        }
    }
    ...
    public void field_caps_staticIndices() throws Exception {
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            assertThat(httpResponse, containsExactly(index_a1).at("indices").but(user.indexMatcher("read")).whenEmpty(403));
        }
    }
        ...
        try (TestRestClient restClient = cluster.getRestClient(user)) {
            ...
            if (clusterConfig == ClusterConfig.LEGACY_PRIVILEGES_EVALUATION) {
                ...
                if (user.indexMatcher("read").covers(alias_ab1)) {
                    ...
                    assertThat(
                        httpResponse,
                        isForbidden("/error/root_cause/0/reason", "no permissions for [indices:data/read/field_caps]")
                    );
                }
            }
        }
