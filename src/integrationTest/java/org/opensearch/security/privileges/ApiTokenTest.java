...
public class ApiTokenTest {
    ...
    @Test
    public void shouldAuthenticateWithApiTokenEndPoint() {
        String apiToken = generateApiToken(TEST_TOKEN_PAYLOAD);
        Header authHeader = new BasicHeader("Authorization", "Bearer " + apiToken);
        authenticateWithApiToken(authHeader, TEST_TOKEN_SUBJECT, HttpStatus.SC_OK);
    }
    ...
    public void shouldNotAuthenticateWithATamperedAPIToken() {
        ...
        Header authHeader = new BasicHeader("Authorization", "Bearer " + apiToken);
        authenticateWithApiToken(authHeader, TEST_TOKEN_SUBJECT, HttpStatus.SC_UNAUTHORIZED);
    }
    ...
    public void shouldNotAllowTokenWhenApiTokensAreDisabled() {
        final String apiToken = generateApiToken(TEST_TOKEN_PAYLOAD);
        final Header apiTokenHeader = new BasicHeader("Authorization", "Bearer " + apiToken);
        authenticateWithApiToken(apiTokenHeader, TEST_TOKEN_SUBJECT, HttpStatus.SC_OK);
        ...
        authenticateWithApiToken(apiTokenHeader, TEST_TOKEN_SUBJECT, HttpStatus.SC_UNAUTHORIZED);
    ...
    public void apiTokenSigningCheckChangeIsDetected() {
        ...
        authenticateWithApiToken(apiTokenOriginalKey, TEST_TOKEN_SUBJECT, HttpStatus.SC_OK);
        ...
        authenticateWithApiToken(apiTokenOriginalKey, TEST_TOKEN_SUBJECT, HttpStatus.SC_UNAUTHORIZED);
        ...
        authenticateWithApiToken(apiTokenOtherKey, TEST_TOKEN_SUBJECT, HttpStatus.SC_OK);

        // Change back to the original signing key and the original key still works, and the new key doesn't
        patchApiTokenConfig(defaultApiTokenConfig());
        authenticateWithApiToken(apiTokenOriginalKey, TEST_TOKEN_SUBJECT, HttpStatus.SC_OK);
        authenticateWithApiToken(apiTokenOtherKey, TEST_TOKEN_SUBJECT, HttpStatus.SC_UNAUTHORIZED);
    }
    ...
    private void authenticateWithApiToken(Header authHeader, String expectedUsername, int expectedStatusCode) {
        try (TestRestClient client = cluster.getRestClient(authHeader)) {
