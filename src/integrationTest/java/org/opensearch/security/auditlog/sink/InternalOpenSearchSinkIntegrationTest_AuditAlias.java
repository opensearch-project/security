 ...
 * }</pre>
 *
 * <h3>Why Separate Test Class?</h3>
 * <p><b>Technical Reason:</b> The {@link LocalCluster} is initialized once per test class
 * via {@code @ClassRule} with immutable node settings. The critical configuration difference is:</p>
 * <pre>
 * Test Class 1 (InternalOpenSearchSinkIntegrationTest):
 *   - No custom node settings
 *   - Sink uses default: plugins.security.audit.config.index = 'security-auditlog-'YYYY.MM.dd
 *   - Result: Date-based indices created automatically (e.g., security-auditlog-2025.01.11)
 *
 * Test Class 2 (This class):
 *   - nodeSettings(Map.of("plugins.security.audit.config.index", "security-audit-write-alias"))
 *   - Sink reads override: plugins.security.audit.config.index = security-audit-write-alias
 *   - Result: Sink targets the alias, not date-based pattern
 * </pre>
 * <p><b>Why Not One Class?</b> Node settings cannot be changed after cluster initialization.
 * Attempting both scenarios in one class would require:</p>
 * <ul>
 *   <li><b>Option 1:</b> Restart cluster mid-test → Extremely slow (~20-30s overhead per restart)</li>
 *   <li><b>Option 2:</b> Conditional test execution → Violates test isolation, creates hidden dependencies</li>
 *   <li><b>Option 3:</b> Mock the configuration → Defeats purpose of integration test</li>
 * </ul>
 * <p>Separate classes ensure clean, isolated testing of distinct code branches with minimal overhead.</p>
 *
...
public class InternalOpenSearchSinkIntegrationTest_AuditAlias {
    ...
    public static void setupAuditAlias() throws Exception {
        try (Client client = cluster.getInternalNodeClient()) {
            ...
            assertThat("Cluster must be operational before alias setup", health.getStatus(), not(equalTo(ClusterHealthStatus.RED)));
            ...
            GetAliasesResponse aliasCheck = client.admin().indices().getAliases(new GetAliasesRequest(AUDIT_ALIAS)).actionGet();

            assertThat("Audit alias must be created in setup", aliasCheck.getAliases().isEmpty(), is(false));
        }
    ...
    public void testMultipleEventsAccumulateInAlias() throws Exception {
        try (Client client = cluster.getInternalNodeClient()) {
            ...
            // Generate 3 distinct events with consistent timing
            generateAuditEvent("_cluster/health");
            generateAuditEvent("_cluster/stats");
