...
public class PointInTimeOperationTest {
	...
	*/
	private static final TestSecurityConfig.User LIMITED_POINT_IN_TIME_USER = new TestSecurityConfig.User("limited_point_in_time_user")
			.roles(new TestSecurityConfig.Role("limited_point_in_time_user")
					.indexPermissions(
	...
	*/
	private static final TestSecurityConfig.User POINT_IN_TIME_USER = new TestSecurityConfig.User("point_in_time_user")
			.roles(new TestSecurityConfig.Role("point_in_time_user")
					.indexPermissions(
							"indices:data/read/point_in_time/create",
							"indices:data/read/point_in_time/delete",
							"indices:data/read/search",
	...
	public static void createTestData() {
		try(Client client = cluster.getInternalNodeClient()) {
			client.index(new IndexRequest().setRefreshPolicy(IMMEDIATE).index(FIRST_SONG_INDEX).id(ID_1).source(SONGS[0].asMap())).actionGet();
			client.index(new IndexRequest().setRefreshPolicy(IMMEDIATE).index(FIRST_SONG_INDEX).id(ID_2).source(SONGS[1].asMap())).actionGet();
			client.index(new IndexRequest().setRefreshPolicy(IMMEDIATE).index(FIRST_SONG_INDEX).id(ID_3).source(SONGS[2].asMap())).actionGet();
			client.admin().indices().aliases(new IndicesAliasesRequest().addAliasAction(new IndicesAliasesRequest.AliasActions(ADD).indices(
					FIRST_SONG_INDEX).alias(FIRST_INDEX_ALIAS))).actionGet();

			client.index(new IndexRequest().setRefreshPolicy(IMMEDIATE).index(SECOND_SONG_INDEX).id(ID_4).source(SONGS[3].asMap())).actionGet();
			client.admin().indices().aliases(new IndicesAliasesRequest().addAliasAction(new IndicesAliasesRequest.AliasActions(ADD).indices(
					SECOND_SONG_INDEX).alias(SECOND_INDEX_ALIAS))).actionGet();
		}
	}
	...
	public void deletePit_negative() throws IOException {
		try (RestHighLevelClient restHighLevelClient = cluster.getRestHighLevelClient(LIMITED_POINT_IN_TIME_USER)) {
			...
			assertThatThrownBy(() -> restHighLevelClient.deletePit(new DeletePitRequest(existingPitId), DEFAULT), statusException(FORBIDDEN));
		}
	}
	...
	public void searchWithPit_positive() throws IOException {
		try (RestHighLevelClient restHighLevelClient = cluster.getRestHighLevelClient(LIMITED_POINT_IN_TIME_USER)) {
			...
			assertThat(searchResponse, searchHitsContainDocumentsInAnyOrder(
					Pair.of(FIRST_SONG_INDEX, ID_1), Pair.of(FIRST_SONG_INDEX, ID_2), Pair.of(FIRST_SONG_INDEX, ID_3)
			));
		}
	}
