...
public class SimulationApiIntegrationTest extends AbstractApiIntegrationTest {
    ...
    static {
        testSecurityConfig
                .user(new TestSecurityConfig.User("test-user").password("password"))
                .roles(new TestSecurityConfig.Role("test_role")
                        .indexPermissions("indices:data/read/*")
                        .on("logs-*")
                )
                .rolesMapping(new TestSecurityConfig.RoleMapping("test_role")
                        .users("test-user")
                );
    }
    ...
    @Test
    public void testSimulate_adminUserRejected() throws Exception {
        withUser(ADMIN_USER_NAME, DEFAULT_PASSWORD, client -> {
            String requestBody = """
            {
              "action": "indices:data/read/search",
            ...
            var response = client.postJson(simulationEndpoint(), requestBody);
            assertThat(response.getStatusCode(), is(HttpStatus.SC_BAD_REQUEST));
            assertThat(response.getTextFromJsonBody("/message"), containsString("Permission simulation for admin user is not allowed"));
        });
    }
