...
public class RoleBasedAccessTest extends AbstractRestApiUnitTest {
	...
	public void testActionGroupsApi() throws Exception {
		...
		response = rh.executeGetRequest("/_opendistro/_security/api/user/admin", encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		 settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertTrue(settings.get("admin.hash") != null);

		// legacy user API, accessible for worf, get complete config
		response = rh.executeGetRequest("/_opendistro/_security/api/user/", encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals("", settings.get("admin.hash"));
		...
		Assert.assertEquals("", settings.getAsList("opendistro_security_role_starfleet_library.backendroles").get(0), "starfleet*");
		...
		response = rh.executeGetRequest("/_opendistro/_security/api/user/admin", encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		...
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertEquals(settings.getAsList("opendistro_security_role_starfleet_captains.cluster").get(0), "cluster:monitor*");
