...
public class AuditApiActionTest extends AbstractRestApiUnitTest {
    ...
    public void testDisabledCategoryOrder() throws Exception {
        ...
        final ObjectNode json = (ObjectNode) objectMapper.readTree(objectMapper.writeValueAsString(auditConfig));

        testPutRequest(json, HttpStatus.SC_OK, true);
        RestHelper.HttpResponse response = rh.executeGetRequest(ENDPOINT, adminCredsHeader);
        List<String> actual = Streams.stream(objectMapper.readTree(response.getBody()).at("/config/audit/disabled_rest_categories").iterator())
    ...
    public void testInvalidDisabledCategories() throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executePutRequest(CONFIG_ENDPOINT, objectMapper.writeValueAsString(auditConfig));
        assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
    ...
    public void testReadonlyApi() throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executeGetRequest(ENDPOINT, adminCredsHeader);
        assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        List<String> actual = Streams.stream(objectMapper.readTree(response.getBody()).get("_readonly").iterator())
                .map(JsonNode::textValue)
                .collect(Collectors.toList());
        ...
        // reset
        ObjectNode json = (ObjectNode) objectMapper.readTree(objectMapper.writeValueAsString(auditConfig));
        testPutRequest(json, HttpStatus.SC_OK, true);
        // change enable_rest readonly property
        testReadonlyBoolean(json, "/audit", "enable_rest");

        // reset
        json = (ObjectNode) objectMapper.readTree(objectMapper.writeValueAsString(auditConfig));
        testPutRequest(json, HttpStatus.SC_OK, true);
    ...
    private void testPutRequest(final JsonNode json, final int expectedStatus, final boolean sendAdminCertificate, final Header... header) throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executePutRequest(CONFIG_ENDPOINT, objectMapper.writeValueAsString(json), header);
    ...
    private void testReadonlyMap(final ObjectNode json, final String config, final String resource) throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executePatchRequest(ENDPOINT, "[{\"op\": \"add\",\"path\": \"" + resourcePath + "\",\"value\": " + objectMapper.writeValueAsString(testMap) + "}]", adminCredsHeader);
        assertEquals(HttpStatus.SC_OK, response.getStatusCode());
    }
    ...
    public void testBadRequest() throws Exception {
        ...
        // incorrect category
        final String jsonValue = DefaultObjectMapper.objectMapper.writeValueAsString(ImmutableList.of("RANDOM", "Test"));
        RestHelper.HttpResponse response;
    ...
    private void testPutAction(final String payload, final int expectedStatus, final Header... headers) throws Exception {
        ...
        if (expectedStatus == HttpStatus.SC_OK) {
            ...
            assertEquals(objectMapper.readTree(payload), objectMapper.readTree(response.getBody()).get("config"));
        }
    }
    ...
    private void testBooleanPatch(final String patchResource, final boolean value, final int expected, final Header... headers) throws Exception {
        ...
        if (expected == HttpStatus.SC_OK) {
            assertEquals(value, objectMapper.readTree(rh.executeGetRequest(ENDPOINT, headers).getBody()).at(patchResource).asBoolean());
        }
    }
    ...
    private void testList(final String patchResource, final List<String> expectedList, final int expectedStatus, final Header... headers) throws Exception {
        final String jsonValue = objectMapper.writeValueAsString(expectedList);

        // make empty
        RestHelper.HttpResponse response = rh.executePatchRequest(ENDPOINT, "[{\"op\": \"add\",\"path\": \"" + patchResource + "\",\"value\": []}]", headers);
        assertEquals(expectedStatus, response.getStatusCode());
        if (expectedStatus == HttpStatus.SC_OK) {
            assertEquals(0, objectMapper.readTree(rh.executeGetRequest(ENDPOINT, headers).getBody()).at(patchResource).size());
        }
        ...
        assertEquals(expectedStatus, response.getStatusCode());
        if (expectedStatus == HttpStatus.SC_OK) {
            final JsonNode responseJson = objectMapper.readTree(rh.executeGetRequest(ENDPOINT, headers).getBody());
            final List<String> actualList = objectMapper.readValue(responseJson.at(patchResource).toString(), new TypeReference<List<String>>(){});
    ...
    private void testMap(final String patchResource, final Map<String, List<String>> expectedMap, final int expectedStatus, final Header... headers) throws Exception {
        final String jsonValue = objectMapper.writeValueAsString(expectedMap);
        ...
        if (expectedStatus == HttpStatus.SC_OK) {
            final JsonNode responseJson = objectMapper.readTree(rh.executeGetRequest(ENDPOINT, headers).getBody());
            final Map<String, List<String>> actualMap = objectMapper.readValue(responseJson.at(patchResource).toString(), new TypeReference<Map<String, List<String>>>(){});
    ...
    public void testPatchRequest() throws Exception {
        ...
        final String payload = objectMapper.writeValueAsString(auditConfig);
        ...
        assertEquals(HttpStatus.SC_OK, response.getStatusCode());
        final JsonNode configNode = objectMapper.readTree(response.getBody()).get("config");

        // verify configs are same
        assertEquals(objectMapper.readTree(payload), configNode);
    }
