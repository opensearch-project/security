...
public class IntegrationTests extends SingleClusterTest {
    ...
    public void testMultiRoleSpan() throws Exception {
        ...
        try (TransportClient tc = getInternalTransportClient()) {                                       
            tc.index(new IndexRequest("opendistrosecurity").type("security").id("config").setRefreshPolicy(RefreshPolicy.IMMEDIATE).source("config", FileHelper.readYamlContent("config_multirolespan.yml"))).actionGet();
