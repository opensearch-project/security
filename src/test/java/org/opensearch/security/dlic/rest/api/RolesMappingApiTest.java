...
public class RolesMappingApiTest extends AbstractRestApiUnitTest {
	...
	@Test
	public void testRolesMappingApiPlugins() throws Exception {
		...
		// check rolesmapping exists, old config api
		HttpResponse response = rh.executeGetRequest("_plugins/_security/api/rolesmapping");
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// check rolesmapping exists, new API
		response = rh.executeGetRequest("_plugins/_security/api/rolesmapping");
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getContentType(), response.isJsonContentType());
		...
		// GET, rolesmapping does not exist
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/nothinghthere", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

		// GET, new URL endpoint in security
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getContentType(), response.isJsonContentType());
		...
		// Super admin should be able to describe particular hidden rolemapping
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/opendistro_security_internal", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("\"hidden\":true"));
		...
		// Non-existing role
		response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/idonotexist", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

		// read only role
		// SuperAdmin can delete read only role
		response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_library", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// hidden role
		response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/opendistro_security_internal", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("'opendistro_security_internal' deleted."));

		// remove complete role mapping for opendistro_security_role_starfleet_captains
		response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_captains", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		response = rh.executeGetRequest("_plugins/_security/api/configuration/rolesmapping");
		rh.sendAdminCertificate = false;
		...
		// remove also opendistro_security_role_starfleet, poor picard has no mapping left
		rh.sendAdminCertificate = true;
		response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		rh.sendAdminCertificate = false;
		checkAllSfForbidden();
		...
		// put with empty mapping, must fail
		response = rh.executePutRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_captains", "", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(AbstractConfigurationValidator.ErrorType.PAYLOAD_MANDATORY.getMessage(), settings.get("reason"));

		// put new configuration with invalid payload, must fail
		response = rh.executePutRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_captains",
										FileHelper.loadFile("restapi/rolesmapping_not_parseable.json"), new Header[0]);
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
		Assert.assertEquals(AbstractConfigurationValidator.ErrorType.BODY_NOT_PARSEABLE.getMessage(), settings.get("reason"));
		...
		// Read only role mapping
		// SuperAdmin can add read only roles - mappings
		response = rh.executePutRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_library",
										FileHelper.loadFile("restapi/rolesmapping_all_access.json"), new Header[0]);
		Assert.assertEquals(HttpStatus.SC_CREATED, response.getStatusCode());

		// hidden role, allowed for super admin
		response = rh.executePutRequest("/_plugins/_security/api/rolesmapping/opendistro_security_internal",
										FileHelper.loadFile("restapi/rolesmapping_all_access.json"), new Header[0]);
		Assert.assertEquals(HttpStatus.SC_CREATED, response.getStatusCode());
		...
		// PATCH on non-existing resource
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/imnothere", "[{ \"op\": \"add\", \"path\": \"/a/b/c\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

		// PATCH read only resource, must be forbidden
		// SuperAdmin can patch read-only resource
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_library", "[{ \"op\": \"add\", \"path\": \"/description\", \"value\": \"foo\"] }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

		// PATCH hidden resource, must be not found, can be found by super admin
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/opendistro_security_internal", "[{ \"op\": \"add\", \"path\": \"/a/b/c\", \"value\": [ " +
				"\"foo\", \"bar\" ] }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

		// PATCH value of hidden flag, must fail with validation error
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_vulcans", "[{ \"op\": \"add\", \"path\": \"/hidden\", \"value\": true }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
		Assert.assertTrue(response.getBody().matches(".*\"invalid_keys\"\\s*:\\s*\\{\\s*\"keys\"\\s*:\\s*\"hidden\"\\s*\\}.*"));

		// PATCH
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_vulcans", "[{ \"op\": \"add\", \"path\": \"/backend_roles/-\", \"value\": \"spring\" }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_vulcans", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		List<String> permissions = settings.getAsList("opendistro_security_role_vulcans.backend_roles");
		Assert.assertNotNull(permissions);
		...
		// PATCH on non-existing resource
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/imnothere/a\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

		// PATCH read only resource, must be forbidden
		// SuperAdmin can patch read only resource
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/opendistro_security_role_starfleet_library/description\", \"value\": \"foo\" }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// PATCH hidden resource, must be bad request
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/opendistro_security_internal/a\", \"value\": [ \"foo\", \"bar\" ] }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());

		// PATCH value of hidden flag, must fail with validation error
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/opendistro_security_role_vulcans/hidden\", \"value\": true }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_BAD_REQUEST, response.getStatusCode());
		Assert.assertTrue(response.getBody().matches(".*\"invalid_keys\"\\s*:\\s*\\{\\s*\"keys\"\\s*:\\s*\"hidden\"\\s*\\}.*"));

		// PATCH
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/bulknew1\", \"value\": {  \"backend_roles\":[\"vulcanadmin\"]} }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/bulknew1", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		permissions = settings.getAsList("bulknew1.backend_roles");
		Assert.assertNotNull(permissions);
		...
		// PATCH delete
		rh.sendAdminCertificate = true;
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"remove\", \"path\": \"/bulknew1\"}]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		response = rh.executeGetRequest("/_plugins/_security/api/rolesmapping/bulknew1", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
		...
		// mapping with several backend roles, one of the is captain
		deleteAndputNewMappingPlugins("rolesmapping_backendroles_captains_list.json");
		checkAllSfAllowed();

		// mapping with one backend role, captain
		deleteAndputNewMappingPlugins("rolesmapping_backendroles_captains_single.json");
		checkAllSfAllowed();

		// mapping with several users, one is picard
		deleteAndputNewMappingPlugins("rolesmapping_users_picard_list.json");
		checkAllSfAllowed();

		// just user picard
		deleteAndputNewMappingPlugins("rolesmapping_users_picard_single.json");
		checkAllSfAllowed();

		// hosts
		deleteAndputNewMappingPlugins("rolesmapping_hosts_list.json");
		checkAllSfAllowed();

		// hosts
		deleteAndputNewMappingPlugins("rolesmapping_hosts_single.json");
		checkAllSfAllowed();

		// full settings, access
		deleteAndputNewMappingPlugins("rolesmapping_all_access.json");
		checkAllSfAllowed();

		// full settings, no access
		deleteAndputNewMappingPlugins("rolesmapping_all_noaccess.json");
		checkAllSfForbidden();
	...
	private HttpResponse deleteAndputNewMappingPlugins(String fileName) throws Exception {
		rh.sendAdminCertificate = true;
		HttpResponse response = rh.executeDeleteRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_captains",
														new Header[0]);
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		response = rh.executePutRequest("/_plugins/_security/api/rolesmapping/opendistro_security_role_starfleet_captains",
										FileHelper.loadFile("restapi/"+fileName), new Header[0]);
	...
	@Test
	public void testRolesMappingApiForNonSuperAdminPlugins() throws Exception {
		...
		// Patch hidden roles mapping
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping/opendistro_security_internal", "[{ \"op\": \"add\", \"path\": \"/description\", \"value\": \"foo\" }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());

		// Patch multiple hidden roles mapping
		response = rh.executePatchRequest("/_plugins/_security/api/rolesmapping", "[{ \"op\": \"add\", \"path\": \"/opendistro_security_internal/description\", \"value\": \"foo\" }]", new Header[0]);
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
