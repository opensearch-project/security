...
public class UserApiTest extends AbstractRestApiUnitTest {
    ...
    public void testSecurityRoles() throws Exception {
        ...
        response = rh.executePatchRequest(ENDPOINT + "/internalusers", "[{ \"op\": \"add\", \"path\": \"/newuser\", \"value\": {\"password\": \"fair password for the user\", \"opendistro_security_roles\": [\"opendistro_security_all_access\"] } }]", new Header[0]);
        Assert.assertEquals(response.getBody(), HttpStatus.SC_OK, response.getStatusCode());
    ...
    public void testParallelPutRequests() throws Exception {
        ...
        HttpResponse[] responses = rh.executeMultipleAsyncPutRequest(10, ENDPOINT + "/internalusers/test1", "{\"password\":\"test1test1test1test1test1test1\"}");
