...
public class RoleBasedAccessTest extends AbstractRestApiUnitTest {
	...
	public void testActionGroupsApi() throws Exception {
		...
		RestHelper.HttpResponse response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/admin", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		...
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/admin", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		 settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertTrue(settings.get("admin.hash") != null);

		// legacy user API, accessible for worf, get complete config
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertEquals("", settings.get("admin.hash"));
		...
		response = rh.executeGetRequest("/_opendistro/_security/api/rolesmapping", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
...
//		Assert.assertTrue(response.getBody().contains("does not have any access to endpoint CONFIGURATION"));
		...
		response = rh.executeDeleteRequest("_opendistro/_security/api/cache", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		...
		response = rh.executeDeleteRequest("_opendistro/_security/api/cache", AbstractSecurityUnitTest.encodeBasicHeader("sarek", "sarek"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("does not have any access to endpoint CACHE"));

		// Admin user has no eligible role at all
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/admin", AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		...
		// Admin user has no eligible role at all
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers", AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("does not have any role privileged for admin access"));

		// Admin user has no eligible role at all
		response = rh.executeGetRequest("/_opendistro/_security/api/roles", AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("does not have any role privileged for admin access"));
		...
		// Admin user has no eligible role at all
		response = rh.executeDeleteRequest("/_opendistro/_security/api/internalusers/admin", AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("does not have any role privileged for admin access"));

		// Worf, has access to internalusers API, able to delete
		response = rh.executeDeleteRequest("/_opendistro/_security/api/internalusers/other", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("'other' deleted"));

		// Worf, has access to internalusers API, user "other" deleted now
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/other", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_NOT_FOUND, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("'other' not found"));

		// Worf, has access to roles API, get captains role
		response = rh.executeGetRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		...
		response = rh.executeDeleteRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		...
		response = rh.executeDeleteRequest("/_opendistro/_security/api/rolesmapping/opendistro_security_unittest_1", AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());
		...
		response = rh.executePutRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains",
				FileHelper.loadFile("restapi/roles_captains_tenants.json"), AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());

		// worf, restore role starfleet captains
		response = rh.executePutRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains",
				FileHelper.loadFile("restapi/roles_captains_different_content.json"), AbstractSecurityUnitTest.encodeBasicHeader("worf", "worf"));
		...
		// admin
		response = rh.executeGetRequest("/_opendistro/_security/api/internalusers/admin", AbstractSecurityUnitTest.encodeBasicHeader("la", "lu"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		settings = Settings.builder().loadFromSource(response.getBody(), XContentType.JSON).build();
		Assert.assertTrue(settings.get("admin.hash") != null);
		Assert.assertEquals("", settings.get("admin.hash"));
		...
		// cache
		response = rh.executeDeleteRequest("_opendistro/_security/api/cache", AbstractSecurityUnitTest.encodeBasicHeader("wrong", "wrong"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		...
		response = rh.executeGetRequest("_opendistro/_security/api/actiongroups", AbstractSecurityUnitTest.encodeBasicHeader("test", "test"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// clear cache - globally disabled, has to fail
		response = rh.executeDeleteRequest("_opendistro/_security/api/cache", AbstractSecurityUnitTest.encodeBasicHeader("test", "test"));
		Assert.assertEquals(HttpStatus.SC_FORBIDDEN, response.getStatusCode());

		// PUT roles
		response = rh.executePutRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains",
				FileHelper.loadFile("restapi/roles_captains_different_content.json"), AbstractSecurityUnitTest.encodeBasicHeader("test", "test"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// GET captions role
		response = rh.executeGetRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains", AbstractSecurityUnitTest.encodeBasicHeader("test", "test"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());

		// Delete captions role
		response = rh.executeDeleteRequest("/_opendistro/_security/api/roles/opendistro_security_role_starfleet_captains", AbstractSecurityUnitTest.encodeBasicHeader("test", "test"));
		Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
		Assert.assertTrue(response.getBody().contains("'opendistro_security_role_starfleet_captains' deleted"));
