...
public class CustomFieldMaskedComplexMappingTest extends AbstractDlsFlsTest{
    ...
    public void testComplexMappingAggregationsRace() throws Exception {
        ...
        HttpResponse res;
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executePostRequest("/logs/_search?pretty&size=0", query, AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"))).getStatusCode());
        ...
        for(int i=0;i<10;i++) {
            Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executePostRequest("/logs/_search?pretty&size=0", query, AbstractSecurityUnitTest.encodeBasicHeader("user_masked_nowc1", "password"))).getStatusCode());
            System.out.println(res.getBody());
        }
        ...
        for(int i=0;i<10;i++) {
            ...
            Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executePostRequest("/logs/_search?pretty&size=0", query, AbstractSecurityUnitTest.encodeBasicHeader("user_masked_nowc", "password"))).getStatusCode());
            System.out.println(res.getBody());
    ...
    public void testComplexMappingSearch() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("/logs/_search?pretty&size=100", AbstractSecurityUnitTest.encodeBasicHeader("admin", "admin"))).getStatusCode());
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("/logs/_search?pretty&size=100", AbstractSecurityUnitTest.encodeBasicHeader("user_masked_nowc", "password"))).getStatusCode());
