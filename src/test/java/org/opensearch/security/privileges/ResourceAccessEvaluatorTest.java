...
public class ResourceAccessEvaluatorTest {
    ...
    public void testPublicResourceAllowed() throws IOException {
        ...
        doAnswer(inv -> {
            ...
            when(doc.isSharedWithEveryone()).thenReturn(true);
    ...
    public void testNotSharedDenied() throws IOException {
        ...
        doAnswer(inv -> {
            ActionListener<ResourceSharing> listener = inv.getArgument(2);
            var doc = mock(ResourceSharing.class);
            when(doc.isSharedWithEveryone()).thenReturn(false);
    ...
    public void testShareWithAllowed() throws IOException {
        ...
        when(rba.flattenedActionGroups()).thenReturn(ag);
        when(context.getActionPrivileges()).thenReturn(rba);
