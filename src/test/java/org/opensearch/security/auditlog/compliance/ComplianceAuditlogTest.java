...
public class ComplianceAuditlogTest extends AbstractAuditlogiUnitTest {
    ...
    public void testExternalConfig() throws Exception {
        ...
        final List<AuditMessage> allMessages = TestAuditlogImpl.doThenWaitForMessages(() -> {
            try {
                setup(additionalSettings);
        ...
        // Filter out transport layer messages
        final List<AuditMessage> messages = allMessages.stream().filter(msg -> msg.getOrigin() != Origin.TRANSPORT).collect(Collectors.toList());
