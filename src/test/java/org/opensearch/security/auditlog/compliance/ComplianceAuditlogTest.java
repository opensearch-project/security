...
public class ComplianceAuditlogTest extends AbstractAuditlogiUnitTest {
    ...
    public void testSourceFilter() throws Exception {
        Settings additionalSettings = Settings.builder()
                ...
                .put(ConfigConstants.OPENDISTRO_SECURITY_AUDIT_CONFIG_DISABLED_REST_CATEGORIES,
                        "authenticated,GRANTED_PRIVILEGES")
                .build();
        ...
        rh.executePutRequest("emp/_doc/0?refresh",
                "{\"Designation\" : \"CEO\", \"Gender\" : \"female\", \"Salary\" : 100}", new Header[0]);
        rh.executePutRequest("emp/_doc/1?refresh",
                "{\"Designation\" : \"IT\", \"Gender\" : \"male\", \"Salary\" : 200}", new Header[0]);
        rh.executePutRequest("emp/_doc/2?refresh",
                "{\"Designation\" : \"IT\", \"Gender\" : \"female\", \"Salary\" : 300}", new Header[0]);
        ...
        final AuditMessage message = TestAuditlogImpl.doThenWaitForMessage(() -> {
            final HttpResponse response = rh.executePostRequest("_search?pretty", search,
    ...
    public void testComplianceEnable() throws Exception {
        ...
        try {
            messages = TestAuditlogImpl.doThenWaitForMessages(() -> {
                rh.executePutRequest("emp/_doc/0?refresh",
                        "{\"Designation\" : \"CEO\", \"Gender\" : \"female\", \"Salary\" : 100}");
                rh.executeGetRequest("_cat/shards?v");
            }, 7);
        } catch (final MessagesNotFoundException ex) {
            // indices:admin/mapping/auto_put can be logged twice, this handles if they were
            // not found
    ...
    public void testSourceFilterMsearch() throws Exception {
        ...
        final List<AuditMessage> messages = TestAuditlogImpl.doThenWaitForMessages(() -> {
            HttpResponse response = rh.executePostRequest("_msearch?pretty", search,
                    encodeBasicHeader("admin", "admin"));
        ...
        final AuditMessage genderMsg = messages.stream().filter(msg -> msg.getRequestBody().contains("Gender"))
    ...
    public void testInternalConfig() throws Exception {
        ...
        final List<String> expectedDocumentsTypes = List.of(
                ...
                "audit");
        final List<AuditMessage> messages = TestAuditlogImpl.doThenWaitForMessages(() -> {
            try (RestHighLevelClient restHighLevelClient = getRestClient(clusterInfo, "kirk-keystore.jks",
                    "truststore.jks")) {
                for (IndexRequest ir : new DynamicSecurityConfig().setSecurityRoles("roles_2.yml")
                        .getDynamicConfig(getResourceFolder())) {
                    restHighLevelClient.index(ir, RequestOptions.DEFAULT);
                    GetResponse getDocumentResponse = restHighLevelClient.get(new GetRequest(ir.index(), ir.id()),
        ...
        final List<String> documentIds = messages.stream().map(AuditMessage::getDocId).distinct()
                .collect(Collectors.toList());
    ...
    public void testExternalConfig() throws Exception {
        ...
        // Record the updated config, and then for each node record that the config was
        // updated
    ...
    public void testUpdate() throws Exception {
        ...
        try (Client tc = getClient()) {
            tc.prepareIndex("humanresources").setRefreshPolicy(RefreshPolicy.IMMEDIATE).setSource("Age", 456).execute()
                    .actionGet();
        }
    ...
    public void testWriteHistory() throws Exception {
        ...
        Assert.assertTrue(
                TestAuditlogImpl.sb.toString().split(".*audit_compliance_diff_content.*replace.*").length == 1);
